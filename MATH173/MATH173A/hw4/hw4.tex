...
\begin{homeworkProblem}
	...
	\begin{enumerate}[(a)]
		...
		\begin{proof}
			...
			Since $x^{(0)}$ only have the first entry non-zero, $x_2^{(k)} = 0$ for all $k$ by induction
			and thus
			\[
				x^{(k + 1)} = \frac{1}{(1 - 2\mu)x^{(k)}_1}\begin{bmatrix} (1 - 2\mu)x^{(k)}_1 \\ 0 \end{bmatrix} = \begin{bmatrix} 1 \\ 0 \end{bmatrix}.
			\]
			But then $f\left(\begin{bmatrix} 1 \\ 0 \end{bmatrix}\right) = 1$ and $f\left(\begin{bmatrix} 0 \\ 1
				\end{bmatrix}\right) = 0$, so the algorithm fails to converge to a minimum.
		\end{proof}
	\end{enumerate}
\end{homeworkProblem}
\end{document}