...
\begin{homeworkProblem}
	...
	\begin{proof}
		...
		\[
			\left\| f\left(x^{(t)}\right) - f\left( x^*\right) \right\| \le \frac{5}{2t\mu} = \frac{15}{2t}.
		\]
		Thus, $t \geq 7.5 \times 10^4$ to satisfy the requirement.
	\end{proof}
	\end{enumerate}
\end{homeworkProblem}
	...
	\begin{enumerate}[(a)]
		...
		\begin{enumerate}[i.]
			...
			\begin{proof}
				...
				\]
				Thus, we would have to take at most $4$ steps of backtracking before accepting $x^{(1)}$.
			\end{proof}
		\end{enumerate}
	\end{enumerate}
\end{homeworkProblem}
