...
\begin{homeworkProblem}
  ...
  \begin{enumerate}[(a)]
    ...
    \begin{proof}
      ...
      \[
        \prob(D_e \geq 2\sqrt{k \log m}) \leq 2\prob\left(R_e \geq \frac{k}{2} + \sqrt{k \log m}\right) \leq 2\exp(-2\log m) = 2m^{-2}
      \]
      But then there are $m$ edges, so 
      \[
        \prob(\operatorname{disc}(H, \chi) \geq 2\sqrt{k \log m}) \leq m\prob(D_e \geq 2\sqrt{k \log m}) \leq \frac{2}{m}.
      \]
    \end{proof}
    ...
    \begin{proof}
      ...
      \]
      By Lemma 3.2, we may construct a dependency digraph $D$ on $A_e$'s by joining $A_e$ to $A_f$ if $e \cap f \neq \emptyset$. Since each $A_e$ is independent of the family of all but at most $d$ other $A_e$'s, the result now follows from LLL.
    \end{proof}
  \end{enumerate}
\end{homeworkProblem}
  ...
  \begin{enumerate}[(a)]
    ...
    \begin{proof}
      ...
      \[
        c\eta e^{-c\eta} = ce^{-c}. 
      \]
      Since $\eta < 1$, we know $d < c$. But then there's no $d \in [1, c)$ that satisfies the equation by part (a). The result now follows from part (a).
    \end{proof}
    ...
    \begin{proof}
      By (b),
      \[
        \prob(X_1 = k \mid \mathbf{X}_{\mathrm{Po}(c)} \text{ extincts}) = \eta^{k - 1}e^{-c}\frac{c^k}{k!} = \eta^{k - 1} \cdot \frac{c^{k - 1}}{k!} \cdot c\eta e^{-c\eta} = \frac{d^k}{k!}e^{-d} = \prob(\mathrm{Po}(d) = k).
      \]
      Condition on extinction. Then the all the subtrees will also go extinct. It follows from induction and part (c) that each vertex in the branching process will have the same distribution as the branching process $\mathbf{X}_{\mathrm{Po}(d)}$.
