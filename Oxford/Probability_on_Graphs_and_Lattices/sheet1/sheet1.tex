...
\begin{homeworkProblem}
  ...
  \begin{proof}
    Reorder the vertices of $G$ by their hitting times, i.e. $i < j$ if and only if $\tau_{v_i} < \tau_{v_j}$. Let $T_n \subseteq G$ be graph with edges
    \[
      E(T_n) = \{\{x_{\tau_{v_i}-1}, x_{\tau_{v_i}}\} : i = 0, \dots, n - 1\}.
    \]
    We prove by induction on $n$ that $T$ is a connected graph on $v_0, \dots, v_{n - 1}$. When $n = 0$, $T_0$ is simply the singleton graph on $v_0$. Suppose $n \geq 1$. Then
    \[
      E(T_n) = \{\{x_{\tau_{v_{n - 1}}-1}, x_{\tau_{v_{n - 1}}}\} \} \cup E(T_{n - 1}).
    \]
    By induction, $T_{n - 1}$ is a connected graph on $v_0, \dots, v_{n - 2}$. But then $x_{\tau_{v_{n - 1}}-1} = v_{k}$ for some $k < n - 1$, and thus $T_n$ is a connected graph on $v_0, \dots, v_{n - 1}$. It now follows from $|E(T_{n})| = |E(T)| = |V(G)| - 1$ that $T$ is a spanning tree of $G$.
  \end{proof}
\end{homeworkProblem}
...
\begin{homeworkProblem}
  \begin{enumerate}[(a)]
    ...
    \begin{proof}
      Let $T_i$ be number of boxes it takes to get the $i$-th distinct coupon after getting the previous $i - 1$ distinct coupons. Then $N_n = \sum_{i = 1}^n T_i$. Note that after getting $i - 1$ distinct coupons, the probability of getting a new coupon is $(n - i + 1)/n$. Thus, $T_i$ is a geometric random variable with parameter $(n - i + 1)/n$, so $\E[T_i] = n/(n - i + 1)$. By the linearity of expectation,
      \[
        \E[N_n] = \sum_{i = 1}^n \E[T_i] = \sum_{i = 1}^n \frac{n}{n - i + 1} = n \sum_{j = 1}^n \frac{1}{j} \sim n \log n.
      \]
    ...
    \begin{proof}
      Consider the vertices of $K_n$ as coupons. Then performing the Aldous-Broder algorithm on $K_n$ is equivalent to collecting the full set of $n$ coupons, except we do not get the same coupon twice in a row as there are no loops in $K_n$. In particular, the probability of getting a new coupon after getting $i - 1$ distinct coupons is $(n - i + 1)/(n - 1) > (n - i + 1)/n$. Thus, the expected number of steps taken by the Aldous-Broder algorithm on $K_n$ is at most the expected number of steps taken by the coupon collector's problem on $K_n$, which is $n \log n$.
    \end{proof}
  \end{enumerate}
\end{homeworkProblem}
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    Let the middle left vertex be $a$ and the middle right vertex be $b$. Let $P_w = \prob_w^G(\tau_v < \tau_u)$ for $w \in V(G) \backslash \{u, v\}$. Let $P_u = \prob_u^G(\tau_v < \tau_u^+)$. We then have
    \[
      \begin{cases}
        P_u = \frac{1}{2} + \frac{1}{2}P_a \\
        P_a = \frac{1}{3}P_b + \frac{1}{3}P_x \\
        P_x = \frac{1}{2}P_a + \frac{1}{2}P_y \\
        P_b = \frac{1}{3} + \frac{1}{3}P_a + \frac{1}{3}P_y \\
        P_y = \frac{1}{2}P_b + \frac{1}{2}P_x \\
      \end{cases}
    \]
    Solving this yields $P_u = 15/22$. For $w \in V(G) \backslash \{v\}$, define $P_w'$ in a similar way but on $G/e'$. We then have
    \[
      \begin{cases}
        P_u = \frac{1}{2} + \frac{1}{2}P_a \\
        P_a = \frac{1}{3}P_b + \frac{1}{3}P_x \\
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    Suppose this collection of directed edges does not contain a directed cycle. Since we are selecting a directed edge pointing outwards from each vertex $v_i \in V(G) \setminus \{v_0\}$, none of the vertices $v_i$ can be a sink in the resulting directed graph $T$. But then $v_0$ is the only possible sink in $T$. Since $T$ is a DAG, it must contain a sink, so $T$ is a DAG with a single sink at $v_0$, which makes $T$ a spanning tree.
  \end{proof}
\end{homeworkProblem}
