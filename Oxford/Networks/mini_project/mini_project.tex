...
\begin{equation}\label{eq:tri_exact}
  \boxed{\E[T] = \frac{2pN}{(1-2p)(1-3p)} + \frac{2A_N}{1-2p} - \frac{2B_N}{1-3p}},
\end{equation}
...
\end{equation}
The homogeneous solution is $F_N = N(N+1)/2$, satisfying $F_{N+1}/F_N = (N+2)/N$. Set $\E[D_N]=\gamma_N F_N$. Then \eqref{eq:D_rec} is reduced to
\begin{equation}
...
\begin{equation}
  \E[D_N]\sim(1-p)\,N(N+1)\ln N.
\end{equation}
Substituting into~\eqref{eq:distance_definition} now yields
\begin{equation}\label{eq:dist_exact}
  \boxed{\ell \sim 2(1-p)\ln N},
\end{equation}
for all $p\in[0,1)$. At $p=1$ the network is $K_N$ with $\ell=1$. Note that the distance scales logarithmically for every $p \in [0, 1)$ with no phase transition, which confirms the small-world property that we established with the $O(1)$ clustering coefficient from Section~\ref{sec:clustering}.
