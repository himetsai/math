...
Let $T$ denote the number of triangles in the network, and write $T_N$ for the number of triangles when the network has $N$ nodes. Triangles are created in an all-or-none network if and only if the ``all'' event occurs and $N \geq 2$. If the target $u$ has degree $k_u$ and there are $e_u$ edges among its neighbours, the new node forms $k_u$ triangles of type $(v_{\text{new}}, u, w_i)$ and $e_u$ triangles of type $(v_{\text{new}}, w_i, w_j)$. Note that $\sum_u k_u = 2M_N$ and $\sum_u e_u = 3T_N$. Averaging over the target choice $u$ thus yields
...
\end{equation}
where $A_N$ is as in~\eqref{eq:edge_exact} and $B_N = \Gamma(N+3p)/[\Gamma(1+3p)\Gamma(N)]$. Thus two regimes emerge:
\begin{itemize}
  \item $p<1/3$: $\E[T]\sim \frac{2p}{(1 -2p)(1 -3p)}N$ (linear).
  \item $p>1/3$: $\E[T]\sim \frac{2}{\Gamma(1+3p)(3p-1)}N^{3p}$ (superlinear).
\end{itemize}
...
\begin{equation}\label{eq:S_rec}
  \E[S_{N+1}] = \left(1+\frac{3p}{N}\right)S_N + 2 + \frac{(4+6p)\E[M_N]}{N}.
\end{equation}
...
\begin{equation}\label{eq:S_exact}
  \E[S] = \frac{(6\!+\!2p)N}{(1\!-\!2p)(1\!-\!3p)} + \frac{(4\!+\!6p)A_N}{p(1\!-\!2p)} - \frac{4(1\!+\!2p)B_N}{p(1\!-\!3p)},
\end{equation}
where $A_N$ and $B_N$ are as in~\eqref{eq:tri_exact}. Recall that $W=(S-2M)/2$, so substituting \eqref{eq:edge_exact} and \eqref{eq:S_exact} yields 
\begin{equation}
  \E[W] = \frac{2(1 + 2p)}{p}\!\left[\frac{pN}{(1 -2p)(1 -3p)} + \frac{A_N}{1 -2p} - \frac{B_N}{1 -3p}\right],
\end{equation}
Comparing with $3\E[T]$ from~\eqref{eq:tri_exact},
\begin{equation}
  3\E[T] = 6\left[\frac{pN}{(1 -2p)(1 -3p)} + \frac{A_N}{1 -2p} - \frac{B_N}{1 -3p}\right],
\end{equation}
we see that $\E[W]$ and $\E[3T]$ have the same bracketed term, and thus we get
\begin{equation}\label{eq:clustering}
  \boxed{\E[C]  \sim \frac{\E[3T]}{\E[W]} = \frac{3p}{1+2p}},
\end{equation}
...
Let $D = \sum_{i<j}d(i,j)$ denote the sum of all pairwise shortest-path distances, and write $D_N$ for this sum when the network has $N$ nodes. The average shortest-path distance is then
...
\begin{itemize}
  \item ``None'' event: the new node $v$ is connected only to the target $u$. Thus, $d(v,u)=1$ and $d(v,j)=1+d(u,j)$ for every $j\neq u$, which yields
  \begin{equation}
    \sum_{j}d(v,j) = N + \sigma_u.
  \end{equation}
  \item ``All'' event: The new node $v$ is adjacent to the target $u$ and all its neighbours. But then $N(u) = N(v)$, so any path from $u$ to $j$ can be rerouted through $v$ by simply replacing the starting point $u$ with $v$. Thus, $d(v,j)=d(u,j)$ for all $j \neq u$, and so
  \begin{equation}
...
\end{equation}
This is a first-order linear recurrence with $\E[D_1]=0$. The homogeneous solution is $\mathcal{H}_N = N(N+1)/2$, satisfying $\mathcal{H}_{N+1}/\mathcal{H}_N = (N+2)/N$. Setting $\E[D_N]=\mathcal{H}_N\,c_N$ reduces~\eqref{eq:D_rec} to
\begin{equation}
  c_{N+1}-c_N = \frac{2(2p-1)}{N+1}+\frac{2(2-3p)}{N+2},
\end{equation}
after partial-fraction decomposition of $2[(1-p)N+p]/[(N+1)(N+2)]$. Telescoping with $c_1=0$ gives
\begin{equation}\label{eq:D_exact}
  \E[D_N] = \tfrac{N(N\!+\!1)}{2}\!\Big[2(2p\!-\!1)(H_N\!-\!1)+2(2\!-\!3p)\!\Big(H_{N+1}\!-\!\tfrac{3}{2}\Big)\Big],
\end{equation}
where $H_N=\sum_{j=1}^{N}1/j$ is the $N$-th harmonic number. Since $H_N\sim\ln N$ for large $N$, the leading behaviour is $c_N\sim 2(1-p)\ln N$, hence $\E[D_N]\sim(1-p)\,N(N+1)\ln N$. Dividing by $\binom{N}{2}$ pairs,
\begin{equation}\label{eq:dist_exact}
  \boxed{\ell(N) \sim 2(1-p)\ln N},
\end{equation}
valid for all $p\in[0,1)$. At $p=0$ this recovers the known result $\ell\sim 2\ln N$ for the random recursive tree; at $p=1$ the network is $K_N$ with $\ell=1$. The distance scales logarithmically for every $p<1$, with no phase transition, confirming the small-world property in conjunction with the $O(1)$ clustering coefficient from Section~\ref{sec:clustering}.
