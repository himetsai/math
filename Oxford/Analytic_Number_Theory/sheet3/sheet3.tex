...
\begin{homeworkProblem}
  ...
  \begin{proof}
    Suppose $s = a + bi$ where $a > 1$. Note that $|1 - w|^{-1} \geq e^{-2|w|}$. Replace $w$ with $p^{-s}$ and we have
    \[
      |\zeta(s)| = \prod_{p} (1 - p^{-s})^{-1} \geq \exp\left(-2\sum_{p} p^{-a}\right).
    \]
    Since $a > 1$, $\exp\left(-2\sum_{p} p^{-a}\right)$ converges to a positive value. The result now follows.
  \end{proof}
\end{homeworkProblem}
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    \]
    On the other hand, notice that for some constant $C$,
    \[
      F_2(x) = C(F_1 * F_1)(x),
    \]
    so 
    \[
      \hat{F_{2}}(\xi) = C\left(\hat{F_{1}}(\xi)\right)^2 = O\left(\frac{1}{\xi^2}\right).
    \]
    Thus,
    \[
      \int_{\infty}^{-\infty} |\hat{F_{2}}(\xi)| \, d\xi \ll \sum_{n = 1}^{\infty} \int_{(n - 1)\pi}^{n\pi} \frac{1}{\xi^2} \, d\xi \ll \sum_{n = 1}^{\infty} \frac{1}{n^2}  < \infty.
    \]
 \end{proof}
\end{homeworkProblem}
  ...
  Let $\chi : \mathbb{N} \to \{-1, 0, 1\}$ be the function defined by 
    ...
    \begin{enumerate}
      ...
      \begin{proof}
        ...
        \[
          \lim_{s \to 1^{+}} \log L(s, \chi) = \lim_{s \to 1^{+}}\sum_p \chi(p)p^{-s} + \lim_{s \to 1^{+}} \sum_p \sum_{k = 2}^\infty \chi(p^k)p^{-ks} < \infty.
        \]
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    By Problem 1 we have $\zeta(0) \neq 0$. By Proposition 3.4, for $s \in (0, 1]$,
    \[
      \zeta(s) = \frac{s}{s - 1} - s\int_1^{\infty} \{x\}x^{-s-1} \, dx.
    \]
    Since $\frac{s}{s - 1} < 0$ and $s\int_1^{\infty} \{x\}x^{-s-1} \, dx > 0$, the equation must not vanish for $s \in [0, 1]$.
  \end{proof}
\end{homeworkProblem}
