...
\end{theorem}
\begin{proof}
  By Theorem~\ref{thm:foc}, $\grad f(x^{*})=0$. Suppose for contradiction that $s\trans\hess f(x^{*})s < 0$ for some $s \in \R^{n}$. Since $s \neq 0$ and $f \in \mathcal{C}^2$, there exists $\hat{\alpha} > 0$ such that $s\trans\hess f(x^{*} + \alpha s)s < 0$ for all $\alpha \in (0, \hat{\alpha}]$. By Taylor's theorem, for any $\alpha \in (0, \hat{\alpha}]$, there exists $t \in (0, \alpha)$ such that
  \[
    f(x^{*} + \alpha s) - f(x^{*}) = \frac{\alpha^2}{2} s\trans\hess f(x^{*} + t s)s < 0,
  \]
  contradiction.
\end{proof}
...
\end{theorem}
\begin{proof}
  If $x^{*}$ is a global minimizer, then by Theorem~\ref{thm:foc}, $\grad f(x^{*})=0$. It remains to show the converse: $\grad f(x^{*})=0$ implies $x^{*}$ is a global minimizer.
...
$\varphi'(0)=\grad f(x^{k})\trans s^{k}<0$. Since $f \in \mathcal{C}^1$, $\varphi'$ is continuous. But then the set $\{\alpha : \varphi'(\alpha) < \beta \varphi'(0)\}$ is open and contains 0, so there exists $\bar{\alpha}>0$ such that
\[
\varphi'(\alpha)< \beta\,\varphi'(0)\qquad\text{for all }\alpha\in[0,\bar{\alpha}].
\]
By the mean value theorem, for each $\alpha\in(0,\bar{\alpha}]$, there exists $\tilde{\alpha}\in(0,\alpha)$ with
\[
\varphi(\alpha)=\varphi(0)+\alpha\,\varphi'(\tilde{\alpha})
<\varphi(0)+\beta\alpha\,\varphi'(0),
\]
which is exactly the Armijo condition \eqref{eq:W1}.
\end{proof}
...
=\beta\varphi'(0) > \eta \varphi'(0)
\]
Thus \eqref{eq:W2} holds at $\alpha^{*}$. And $\alpha^{*}<\bar{\alpha}$, so $\varphi(\alpha^{*})\le\ell(\alpha^{*})$, i.e.\ \eqref{eq:W1} holds as well.
\end{proof}
...
\end{lemma}
\begin{proof}
  First note that $\alpha_{k,\max} > 0$ as $\beta < 1$ and $s^{k}$ is a descent direction. Let $s^{k}$ be a descent direction and let $\alpha \in [0, \alpha_{k,\max}]$. By Lemma~\ref{lem:descent-lemma}, 
  \begin{align*}
    f(x^{k} + \alpha s^{k}) 
    &\leq f(x^{k}) + \alpha \grad f(x^{k})\trans s^{k} + \alpha^2 L\norm{s^{k}}^2 \\
    &\leq f(x^{k}) + \alpha \grad f(x^{k})\trans s^{k} + \alpha(\beta - 1)\grad f(x^{k})\trans s^{k}  \\
...
\begin{proof}
  ...
  \[
    \alpha_{k} = \alpha^{(i)} = \tau\alpha^{(i - 1)} > \tau\alpha_{k,\max}. \qedhere
  \]
\end{proof}
...
\begin{proof}
  ...
  \]
  On the other hand, for $k \in \mathcal{K}_2$ we have $\alpha_{k} \geq \alpha^{(0)}$, so
  \[
...
\end{theorem}
\begin{proof}
  Write $g(x):=\grad f(x)$ and $H(x):=\hess f(x)$. Since $H(x^{*})$ is nonsingular and locally Lipschitz continuous and $H$ is continuous,
  there is a radius $\rho>0$ such that for $x, y \in N(x^{*},\rho)$,
  \begin{itemize}
    \item $H(x)^{-1}$ exists and $\norm{H(x)^{-1}}\le M$ for some $M>0$,
    \item $\norm{H(y)-H(x)}\le L\norm{y-x}$.
  \end{itemize}
  For $x\in N(x^{*},\rho)$, Taylor's theorem gives
  \[
    g(x^{*})=g(x)+H(x)(x^{*}-x)+r(x),
  \]
  ...
  \begin{align*}
    \|r(x)\|
    &= \|g(x + h) - g(x) - H(x)h\| \\
    &\leq \int_{0}^1 \|H(x + th) - H(x)\|\cdot\|h\| \, dt \\
    &\leq L\|h\|^2\int_{0}^1 t \, dt = \frac{L}{2}\|x^* - x\|^2,
  \end{align*}
  by Lipschitz and $h := x^* - x$.
  ...
  \[
    ...
    \le \frac12\norm{x^{k}-x^{*}}.
  \]
  Therefore, $x^{k}\to x^{*}$ and the first inequality gives quadratic convergence.
\end{proof}
...
Let $f\in\mathcal{C}^{1}(\R^{n})$ be bounded below and $\grad f$ Lipschitz continuous. Apply GLM with $s^{k}$ from \eqref{eq:Bksk} and bArmijo linesearch
...
\end{theorem}
\begin{proof}
  Write $H_k = \hess f(x^{k})$ and $g_k = \grad f(x^{k})$. Since $B_k \succ 0$, we may write $s^{k} = -B_k^{-1}g_k$. Assume that $g_k \neq 0$ for all $k \geq 0$. Since $f$ is bounded below and $\grad f$ is Lipschitz continuous, Theorem~\ref{thm:GLM-global} gives
  \begin{equation}\label{eq:Bk-global-condition}
    \lim_{k\to\infty}\min\!\left\{\frac{|g_k\trans s^{k}|}{\norm{s^{k}}}, |g_k\trans s^{k}|\right\}=0.
  \end{equation}
  It remains to show that $\|g_k\| \to 0$ as $k \to \infty$.
...
Let $f\in\mathcal{C}^{2}(\R^{n})$ be bounded below with $\grad f$ $L$-Lipschitz continuous. Suppose the eigenvalues of $\hess f(x)$ satisfy
