...
\paragraph{Interpretation.} The secant equation requires the new model to predict
correctly the change in gradient: in the quadratic case $f(x)=g\trans x+\tfrac12
x\trans Hx$, $\grad f(x^{k+1})-\grad f(x^{k})=H(x^{k+1}-x^{k})$, so $B_{k+1}=H$
satisfies the equation. More generally, the change in gradient encodes
information about the Hessian (cf.\ finite differences).
...
\paragraph{Strategy.} The characterisation below shows that solving \eqref{eq:TRsub}
reduces to the secular equation $\norm{(B_{k}+\lambda I)^{-1}\grad f(x^{k})}=\Delta_{k}$
...
\begin{remark}[Two cases for the global solution]
Theorem~\ref{thm:TR-charac} reduces practical solution to two cases:
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  \item \textbf{Case 1:} $B_{k}\succ 0$ and the unconstrained Newton step
        $s=-B_{k}^{-1}\grad f(x^{k})$ satisfies $\norm{s}\le\Delta_{k}$ — then
        $s^{*}=s$ and $\lambda^{*}=0$ (Newton step is inside the trust region).
  \item \textbf{Case 2:} otherwise — find $\lambda\ge\max\{0,-\lambda_{\min}(B_{k})\}$
