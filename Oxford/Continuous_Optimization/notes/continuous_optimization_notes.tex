...
\begin{proof}
  ...
  \textit{Step 2: $c(x^*) = 0$.}
  Note that $c(x^k) = \sigma_k (u^k - y^k)$. If $u_k$ is bounded and $\sigma_k \to 0$, then $c(x^k) \to 0$. If $\sigma_k$ is bounded and $u_k \to y^*$, then $c(x^k) \to 0$. Hence $c(x^k) = 0$, and so $c(x^*) = 0$ by continuity of $c$.
