...
\begin{homeworkProblem}
  \begin{enumerate}[(a)]
    ...
    \begin{proof}
      ...
      call of size $n/2$, there are $\log_2 n$ levels of recurrence, with $1$ recursive call per
      level. It now follows that
      \begin{align*}
        R(n) 
        &= R(n/2) + O(\log n) \\
        &= \left(R(n/4) + O(\log \frac{n}{2})\right) + O(\log n) \\
        &= O\left(\sum_{k = 0}^{\log n} \log \frac{n}{2^k}\right) \\
        &= O\left(\sum_{k = 0}^{\log n} (\log n - k)\right) \\
        &= O\left(\log^2 n - \sum_{k = 0}^{\log n} k\right) \\
        &= O\left(\log^2 n - \frac{(\log n + 1)\log n}{2}\right) \\
