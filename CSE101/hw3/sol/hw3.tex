...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    We now give a justification for the correctness of the algorithm. Note that at any moment, there
    is always a valid sequence of transfers such that the number of transfers is equal to $time(v)$
    in any iteration if $time(v) < \infty$. We may find such sequence by backtracing $prev(v)$ until
    we reach $X$, and step 11 ensures each transfer in the sequence is valid. Hence, it remains to
    show that $time(v)$ is minimum by the end of the algorithm.
  ...
  \begin{proof}
    ...
    We now give a justification for the correctness of the algorithm. Note that at any moment, there
    is always a valid sequence of transfers such that the number of transfers is equal to $step(v)$
    in any iteration if $step(v) < \infty$. We may find such sequence by backtracing $prev(v)$ until
    we reach $X$, and step 13 ensures each transfer in the sequence is valid. Hence, it remains to
    show that $step(v)$ is minimum by the end of the algorithm.
    ...
    \]
    But then the algorithm picked $u$ over $s_i$ at step 12, so $step(u) < step(s_{i})$,
    contradiction.
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    each vertex $v \neq s$ has at most $d$ nonvisited neighbors (excluding the preceding vertex),
    there are less than $d^k$ vertices that has distance $k$ from $s$ by induction on $k$. But then
    there are less than
    \[
      1 + \sum_{k = 1}^l d^k = \frac{d^{l + 1} - 1}{d - 1}
    \]
    vertices which have distance at most $l$ from $s$. Hence, in the worst case, $t$ would be
    explored before the $\frac{d^{l + 1} - 1}{d - 1}$th vertex is explored, and thus the algorithm
    has a worst case runtime of 
    \[
  ...
  \begin{proof}
    ...
    $t$, say vertex $v$. But then the shortest paths between $s, v$ and $s, t$ have length $l/2$. By
    (a).ii, the runtime for each \texttt{BFS2} is $O(d^{l/2})$, and thus the total runtime is
    $O(2d^{l/2}) = O(d^{l/2})$. 
  \end{proof}
  \end{enumerate}
  \end{enumerate}
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    1}$ or $C_{n - 1}$ from $A_n$, $maxdist(B_{n - 1}) = 2^{n - 1}$ and $maxdist(C_{n - 1}) = 2^{n}
    + n$, both of which are at most $2^n + n$. There are two possible paths from $A_n$ to $A_{n -
    1}$, namely $A_nB_{n - 1}A_{n - 1}$ and $A_nC_{n - 1}A_{n - 1}$, which have weights $2^{n - 1}$
    and $0$, respectively. Since $2^{n - 1} < 2^n + n$, $maxdist(A_{n - 1})$ would be set to $2^{n -
    1}$. But then $2^{n - 1} < 2^n + n$, so the algorithm would prioritize $A_{n - 1}$ over $C_{n -
    1}$. Note that the subgraph starting from $A_{n - 1}$ is isomorphic to $H(n - 1)$. Call it $H'(n
    - 1)$. For $v \in H'(n - 1)$, $maxdist(v)$ can be calculated by running Dijkstra's on $H'(n -
    1)$ starting from $A_{n - 1}$, with the initial distance at $A_{n - 1}$ set to $maxdist(A_{n -
    1})$. By induction,
    \[
  ...
  \begin{proof}
    ...
    initially be assigned $2^{n - 1} < 2^n + n$, $A_{n - 1}$ would be prioritized over $C_{n - 1}$.
    ...
    prioritized over $C_{n - 1}$. Hence, the process of the algorithm before ejecting $C_{n - 1}$ is
    equivalent to running Dijkstra's algorithm on $H'(n - 1)$ starting at $A_{n - 1}$, which would
    ejected $A_0$ $2^{n - 1}$ times, by induction. But then after ejecting $C_{n - 1}$, the
    algorithm push $A_{n - 1}$ back to the priority queue, and the remaining process is also
    equivalent to running Dijkstra's algorithm on $H'(n - 1)$ starting at $A_{n - 1}$, which would
    eject $A_0$ another $2^{n - 1}$ times. Hence, $A_0$ would be ejected $2^n$ times in total.
  \end{proof}
