...
\begin{homeworkProblem}
  ...
  \begin{proof}
    Assume that $D[n]$ is reachable from $D[0]$. Let $OS = \{y_1, \dots, y_{m} = n\}$ be any
    solution which reaches $D[n]$. Suppose for the sake of contradiction that $D[y_i] + x[y_i] >
    D[x_i] + x[x_i]$ for some $i$. Let $l$ be the smallest such index. We then have
    \begin{gather*}
