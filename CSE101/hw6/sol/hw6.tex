...
\begin{homeworkProblem}
	...
	$w(u, v) = 0$ if $v = n$. We now run Dijkstra's algorithm on $G$ starting on vertex 0 and return
	the resulting price at vertex $n$. \\

	\textbf{Runtime Analysis:}

	$G$ has $O(n)$ vertices and $O(n^2)$ edges, so constructing $G$ takes $O(n^2)$ time. Running
	Dijkstra on $G$ using array as a priority queue takes $O(n^2)$ time. In total, the algorithm runs
	in $O(n^2)$ time.
