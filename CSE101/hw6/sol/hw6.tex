...
\begin{quote}
	...
	\begin{quote}
		...
		\[
			A[i] = \begin{cases}
				...
				1 + \min_{s \in S} A[i - s] &\text{ otherwise}
			\end{cases}.
		\]
	\end{quote}
	...
	\begin{quote}
		\begin{enumerate}[1.]
		\item
		Initialize array $A$ of size $n$, with $A[i] = i$ \quad (with 1-based indexing)
		\item
		...
		\item
		\ind\ind\ind $A[i] = 1 + \min(A[i], A[i - s^2])$
		\item 
		{\bf return} $A[n]$
		\end{enumerate}
	\end{quote}
	...
	\begin{quote}
		...
		i$, for all $i \in \Z^+$. Hence, for each positive integer $i \leq n$, looping through all
