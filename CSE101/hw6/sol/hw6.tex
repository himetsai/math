...
\begin{proof}
	...
	$n$ is not a square number, otherwise $A[n] = 1 = \delta(n)$ and we are done. Hence, we have
	$\delta(n) > 1$. Let $c_1, \dots, c_{\delta(n)}$ such that each $c_j$ is a square number and $c_1
	+ \cdots + c_{\delta(n)} = n$. By induction, $A[n - c_{\delta(n)}] = \delta(n - c_{\delta(n)})$.
	But then note that $\delta(n) \geq \delta(n - c_{\delta(n)}) + 1$, otherwise $c_1, \cdots,
	c_{\delta(n) - 1}$ sums up to $n - c_{\delta(n)}$ with less than $\delta(n - c_{\delta(n)})$
	coins, contradiction. Since $A[n]$ is the minimum of $A[n - s]$ for all square number $s$, we have
