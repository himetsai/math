...
\begin{proof}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  The graph is obviously strongly connected. Suppose that the algorithm starts at vertex $A$. Upon
  exploring, $B, C$ are pushed to stack $F$, and not vertex has state $U$ at this point. Hence, both
  of the outgoing edges from $B$ or $C$ lead to reached vertices, so $c$ increments by 4 and the
  algorithm terminates. But then there are 5 cycles in the graph, namely $A \to B \to C \to A$, $A
  \to C \to B \to A$, $A \to B \to A$, $A \to C \to A$, $B \to C \to B$. 
\end{proof}
\end{homeworkProblem}
