...
\begin{homeworkProblem}
  \begin{enumerate}[(a)]
    ...
    \begin{proof}
      ...
      \end{gather}
      for $n < 0$. Rearranging (1), we know $b_n$ follows (2) for $n \in \Z$. Hence, it remains to
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    $dp^d$ terms. Suppose that $s_d$ initially appeared at $a_ka_{k + 1}\ldots a_{k + d - 1}$ and
    reappeared at $a_la_{l + 1}\ldots a_{l + d - 1}$. Since 
    \begin{gather}
      a_{n} = \frac{1}{c_d}(a_{n + d} - c_1a_{n + d - 1} - \dots - c_{d - 1}a_{n + 1}),
    \end{gather}
    we may inductively calculate the first $k$ terms based on $a_k, a_{k + 1}, \ldots ,a_{k + d -
    1}$. Let $s_{d_0}$ be the string of the first $d$ terms. We reapply $(3)$ on $a_la_{l + 1}\ldots
    a_{l + d - 1}$ and get that $s_{d_0}$ repeats at $a_{l - k}a_{l - k + 1}\ldots a_{l - k + d -
    1}$. Hence, $(a_n)_{n \geq 0}$ is periodic.
  \end{proof}
\end{homeworkProblem}
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      \end{bmatrix} \\
      &= \det \begin{bmatrix}
        r_2 - r_1 & r_2(r_2 - r_1) & \cdots & r_2^{d - 2}(r_2 - r_1) \\
        \vdots & \vdots & \ddots & \vdots \\
        r_{d - 1} - r_1 & r_{d - 1}(r_{d - 1} - r_1) & \cdots & r_{d - 1}^{d - 2}(r_{d - 1} - r_1) \\
        1 & 2r_{d-1} - r_1 & \cdots & dr_{d-1}^{d - 1} - (d - 1)r_1r^{d - 2}_{d - 1} \\
      \end{bmatrix} \\
      &= (r_{d-1} - r_1)\left(\prod_{1 \leq i \leq d - 1} (r_{i + 1} - r_1)\right)\det \begin{bmatrix}
        1 & r_2 & r_2^2 & \cdots & r_2^{d - 2} \\
        \vdots & \vdots & \ddots & \vdots \\
        1 & r_{d - 1} & r^2_{d - 1} & \cdots & r_{d - 1}^{d - 2} \\
        0 & 1 & 2r_{d - 1} & \cdots & (d - 1)r_{d - 1}^{d - 2} \\
      \end{bmatrix} \\
      &= (r_{d-1} - r_1)\left(\prod_{1 \leq i \leq d - 1} (r_{i + 1} - r_1)\right)M(r_2, r_3, \dots, r_{d - 1}).
    \end{align*}
