...
\begin{homeworkProblem}
  ...
  \]
  \begin{proof}
    We first show that the improper integral is well-defined. Pick $\epsilon > 0$. There exists $A$
    such that $|f(x) - 1| < \epsilon$ for all $x \geq A$. For $n > A$, 
    \[
    ...
    \]
    and so $\int_0^{\infty} e^{-tx} f(x) \, dx = \int_0^{A} e^{-tx} f(x) \, dx + \int_A^{\infty}
    e^{-tx} f(x) \, dx$ exists. Since $|e^{-tx}| < 1$ for all $t > 0$, 
    \[
