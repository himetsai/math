...
\begin{homeworkProblem}
  ...
  \]
  In addition, by Theorem 6.13,
  \[
    \left|\int_0^{\infty} \frac{\sin x}{(1 + x)^2} \, dx\right| \leq \int_0^{\infty} \left|\frac{\sin x}{(1 + x)^2}\right| \, dx \leq \int_0^{\infty} \frac{1}{(1 + x)^2} \, dx.
  \]
  But then by exercise 8, $\int_0^{\infty} \frac{1}{(1 + x)^2} \, dx$ converges, as $\sum_{n =
  0}^{\infty} \frac{1}{(1 + x)^2}$ converges. and then idk.
\end{homeworkProblem}
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    on $[a, b]$. In addition, on every interval $[x_1, x_{i + 1}]$, since $f(x_1) \leq g(t) \leq
    f(x_{i + 1})$, we have $m_i \leq g(t) \leq M_i$ for all $t \in [x_1, x_{i + 1}]$.
    But then
    \begin{align*}
      \|f - g\|_2^2
      &= \int_a^b |f - g|^2 \\
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    \end{align*}
    It remains to show that equality cannot hold in this case. Suppose that it does. By exercise
    10(a), we have 
    \[
      \frac{[f'(t)]^2}{\int_a^b [f'(x)]^2 \, dx} = \frac{(tf(t))^2}{\int_a^b (xf(x))^2 \, dx},
    \]
    for all $t \in [a, b]$. idk.
  \end{proof}
\end{homeworkProblem}
