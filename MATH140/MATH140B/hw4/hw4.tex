...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    \[
      U(P_1, \alpha(x_i) g(t_i)) - L(P_1, \alpha(x_i) g(t_i)) < \epsilon,
    \]
    \[
      U(P_2, G, \alpha) - L(P_2, G, \alpha) < \epsilon.
    \]
    Let $P = P_1 \cup P_2$. By Theorem 6.7,
    \begin{align*}
      &\left|G(b)\alpha(b) - G(a)\alpha(a) - \int_a^b G \, d\alpha - \int_a^b \alpha(x)g(x) \, dx\right| \\
      &\leq \left|\left|\sum_{i=1}^n G(x_{i-1}) \Delta \alpha_i - \int_a^b G \, d\alpha\right| - \left|\sum_{i=1}^n \alpha(x_i)g(t_i) \Delta x_i - \int_a^b \alpha(x)g(x) \, dx\right|\right| < \epsilon.
    \end{align*} 
  \end{proof}
\end{homeworkProblem}
