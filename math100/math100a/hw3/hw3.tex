...
\begin{homeworkProblem}
    ...
    \begin{proof}
        Consider the function $f: S \rightarrow T$, $f(Hx) = x^{-1}H$, for $x \in G$. 
        Let $Ha, Ha' \in S$, such that $Ha = Ha'$. This implies that $Haa'^{-1} = H$, and so $aa'^{-1} \in H$.
        Let $h = aa'^{-1}$. We know $a^{-1}h = a^{-1}aa'^{-1} = a'^{-1} \in a^{-1}H$, and thus $f(Ha) = a^{-1}H = a'^{-1}H = f(Ha')$.
        ...
        Let $h = ba^{-1} \in H$. We then get $ha = b \in Ha$, and thus $Ha = Hb$. 

        We now show $f$ is surjective.
        For all $y = bH \in T$, we have $x = Hb^{-1} \in S$, so that $f(x) = bH$.
...
\begin{homeworkProblem}
    ...
    \begin{proof}
        ...
        Let $x = aha^{-1}, x' = ah'a^{-1}$, for some $h, h'$, such that $x = x'$. By cancellation, we know $h = h'$. 
        Then, $f(x) = a^{-1}xa = h = h' = a^{-1}x'a = f(x')$, and so $f$ is well defined.
...
\begin{homeworkProblem}
    Let $G$ be an abelian group of order $n$, and $a_1, \dots, a_n$ its elements.
    Let $x = a_1a_2\dots a_n$. Show that:
    \begin{enumerate}
        \item[(a)] If $G$ has exactly one element $b \neq e$ such that $b^2 = e$, then $x = b$.
        \begin{proof}
            In problem $7$, we proved that we can rearrange $x = a_1a_2\dots a_n$ into any ordering.
            For all $a_i^2 \neq e$, we rearrange $a_1a_2\dots a_n$ such that $a_i$ is next it its inverse, 
            which allows each $a_i$ to pair up with its neighboring inverse and resolve to $e$.
            Thus, the sequence becomes $x = be = e$, and we are done.
        \end{proof}
...
\begin{homeworkProblem}
    “Every countable group is finitely generated.” True or False? If true
    then give a proof and if false then give a counterexample.

    \begin{proof}
        Consider $\Q$ under addition. We know $\Q$ is countable.
        Suppose for sake of contradiction that there exists a finite set $S = \{s_1, s_2, \dots, s_n\} \subset \Q$, $s_i = \frac{a_i}{b_i}$ for $a_i, b_i \in \Z - \{0\}$,
        such that $\langle S \rangle = \Q$. 
        Let $b = \Pi_{s_i \in S} b_i$. Then, all elements in $\langle S \rangle$ can be represented in the form of $\frac{c_i}{b}$, $c_i \in \Z$.
        However, we can find $\frac{p}{q} \in \Q$, such that $p, q \in \N$, $\gcd(q, b) = 1$, so that $\frac{p}{q}$ cannot be represented in the form of $\frac{c_i}{b}$, contradiction.
        Therefore, $\Q$ under addition is a countable group that cannot be finitely generated.
    \end{proof}
\end{homeworkProblem}
