...
\begin{proof}
    We restate that $S = \{(x,y) \in \R^2 \}$, $f, g \in A(S)$ such that $f(x, y) = (-x, y)$ and $g(x, y) = (-y, x)$, and $G = \{f^ig^j \, | \, i = 0, 1; j = 0, 1, 2, 3 \}$. 
    Note that since $f$ is a reflection and $g$ is a $90^{\circ}$ rotation, both $f^k = f^{(k \mod 2)}$ and $g^l = g^{(l \mod 4)}$ are in $G$, for $k,l \in \Z$. And also note that $g^4 = f^2 = $ identity mapping $e$.
