...
\begin{homeworkProblem}
  ...
  \begin{proof}
    Consider the function $f: \sym_n \to \sym_n$ which swaps $w(1)$ and $w(2)$ if $w(1) = 1$, and swaps $w(1)$ and $w^{-1}(1)$ if $w(1) \neq 1$. Let $w \in \sym_n$. Observe that $f(f(w)) = 1$, so $f$ is an involution. Suppse $w(1) = 1$. Then applying $f$ to $w$ increases the number of inversions by $1$. Also, $f$ merges the 1-cycle $(1)$ with the cycle containing $w(2)$, so $f$ reduces the number of cycles of $w$ by $1$. Now suppose $w(1) \neq 1$. Then applying $f$ to $w$ decreases the number of inversions by $1$. Also, $f$ splits the the cycle which contains $w(1)$, so $f$ increases the number of cycles of $w$ by $1$. Hence, $f$ changes the parity of $\operatorname{inv}(w)$ and $\operatorname{cyc}(w)$. Thus,
    \[
