...
\begin{homeworkProblem}
  ...
  \begin{proof}
    Denote the hyperplane $x_1 + x_2 + \cdots + x_n = 0$ as $H_0$. Let $\mathcal{A}' \subset \mathcal{A}$ be the Braid arrangement, and let
    \[
      \mathcal{A}'' = \{H_0 \cap H \neq \emptyset : H_0 \not\subseteq H, H \in \mathcal{A}\} = \{H_0 \cap \{x_i = x_j\} \mid 1 \leq i < j \leq n\}.
    \]
    By the Deletion-Rejection principle, we have
    \[
      \chi_{\mathcal{A}}(t) = \chi_{\mathcal{A}'}(t) - \chi_{\mathcal{A}''}(t).
    \]
    We already know the characteristic polynomial of the Braid arrangement is 
    \[
      \chi_{\mathcal{A}'}(t) = t(t - 1)(t - 2) \cdots (t - n + 1).
    \]
    We now use the finite field method to compute $\chi_{\mathcal{A}''}(t)$. Let $q$ be a prime power and let $\mathbb{F}_q$ be the finite field with $q$ elements. The finite field method yields
    \[
      \chi_{\mathcal{A}''}(t) = \# \left(\F_q^n \backslash \bigcup_{H^{(p)} \in \mathcal{A}''^{(p)}} H^{(p)}\right) = \# \left(\F_q^n \backslash \bigcup_{\{x_i = x_j\} \in \mathcal{A}'^{(p)}} H_0^{(p)} \cap \{x_i = x_j\}\right)
    \]
