...
\begin{homeworkProblem}
  \begin{enumerate}[(a)]
    ...
    \begin{proof}
      ...
      and thus
      \[
        \E_{A \backslash \{a_{ij}\}} [N_{ij}(A, X) \mid X, a_{ij}] = \sum_{m \in [n]} \prob[m \notin (R_{ij}(A) \cup C_{ij}(A))] = n(1 - a_{ij})^2.
      \]
      Hence,
      \[
        \E_{a_{ij}}[\log_2 \E_{A \backslash \{a_{ij}\}} [N_{ij}(A, X) \mid X, a_{ij}]] = \int_{0}^1 \log_2 n(1 - x^2) \, dx = \log_2(n) - 2.
      \]

      It now follows that
      \[
        H(X) \leq \E_X \sum_{i, j \in [n]} \E_{a_{ij}}[\log_2 n(1 - a_{ij}^2)] \leq n^2(\log_2(n) - 2),
      \]
      and thus the result. (I don't know where I lost the $o(1)$ term.)
    \end{proof}
    ...
    \begin{proof}
      ...
      \begin{itemize}
        ...
        \item $S_{ij}(A) = \{X_{kl}: a_{kl} < a_{ij} \text{ and } X_{kl} \text{ in the same square
        as } X_{ij}\}$.
      \end{itemize}
      ...
      \[
        \log_2 L(n) = H(X) = \E_A\sum_{i, j \in [n]} H(X_{ij} \mid X_{kl}: a_{kl} < a_{ij}).
      \]
      Let $N_{ij}(A, X) = |[N] - R_{ij}(A) \cup C_{ij}(A) \cup S_{ij}(A)|$. Rewriting the entropy in
      expectation form, 
      \[
        H(X_{ij} \mid X_{kl}: a_{kl} < a_{ij}) = \E_X[\log_2 N_{ij}(A, X)],
      \]
      ...
      For $m \in [N]$, notice
      \[
        \prob[m \notin (R_{ij}(A) \cup C_{ij}(A) \cup S_{ij}(A))] = \prob(m \notin R_{ij}(A))\prob(m \notin C_{ij}(A))\prob(m \notin S_{ij}(A)) = (1 - a_{ij})^3,
      \]
      and thus
      \[
        \E_{A \backslash \{a_{ij}\}} [N_{ij}(A, X) \mid X, a_{ij}] = \sum_{m \in [N]} \prob[m \notin (R_{ij}(A) \cup C_{ij}(A) \cup S_{ij}(A))] = N(1 - a_{ij})^3.
      \]
      Hence,
      \[
        \E_{a_{ij}}[\log_2 \E_{A \backslash \{a_{ij}\}} [N_{ij}(A, X) \mid X, a_{ij}]] = \int_{0}^1 \log_2 N(1 - x^2) \, dx = \log_2(N) - 3.
      \]

      It now follows that
      \[
        H(X) \leq \E_X \sum_{i, j \in [n]} \E_{a_{ij}}[\log_2 n(1 - a_{ij}^2)] \leq N^2(\log_2(N) - 3),
      \]
      and thus the result. (I don't know where I lost the $o(1)$ term.)
    \end{proof}
  \end{enumerate}
