...
\begin{homeworkProblem}
	Let $\gamma$ be a closed rectifiable curve in an open set $G$ and $a \notin G$. Show that for $n
	\geq 2$, $\int_{\gamma} (z - a)^{-n} \, dz = 0$.

	\begin{proof}
		Let $\alpha$ be the start/end point of $\gamma$. Since $a \notin G$, the primitive of $(z -
		a)^{-n}$ is $\frac{1}{n - 1}(z - a)^{-(n - 1)}$. By theorem 1.18, 
		\[
...
\begin{homeworkProblem}
	...
	\begin{proof}
		...
		\overline{B_{r}(z_0)}$ with $d(w, z) < \delta'_r$. Define path $\sigma_z: [0, 1] \to
		\overline{B_{r}(z_0)}$ as $\sigma_z(t) = tz + (1 - t)z_0$ and note that $\sigma_z$ is
		rectifiable, with $\int_{\sigma_z} |dw| = z - z_0$. Then for all for $s \in \{\gamma\}$ and
		$d(z, z_0) < \delta'_r$, 
		\begin{gather}
