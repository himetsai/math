...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    circle is less than the circumference of the circle. Hence, $f_k$ is continuous for all $k$. By
    theorem 5.15, $f_k$ is uniformly continuous, so there exists $\delta$ such that for all $d(z, w)
    < \delta$, we have $|f_k(z) - f_k(w)| < d(\C \backslash G, P)$. We may now partition $[z_k, z_{k
    + 1}]$ into finitely many intervals of length less than $\delta$, with endpoints $z_k = w_0,
    w_1, \ldots, w_m = z_{k + 1}$. Since $|f_k(w_i) - f_k(w_{i + 1})| < d(\C \backslash G, P)$ for
    all $i$,
    \[
...
\begin{homeworkProblem}
  ...
  \begin{proof}
    ...
    However, this does not work in the general case. Given any real function $f$ on interval $[a,
    b]$ which is not Lipschitz, there exists a sequence of polynomials $\{p_n\}$ on $[a, b]$ such
    that $p_n \to f$ uniformly, by the Weierstrass approximation theorem. Since $\sup_{x \in [a, b]}
    |p_n'(x)| < \infty$ for all $n$, 
    \[
