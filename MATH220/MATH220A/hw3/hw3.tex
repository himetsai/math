...
\begin{homeworkProblem}
  ...
  \begin{proof}
    We may denote $P = [z_1, z_2] \cup \cdot [z_{n}, z_{n + 1}]$, where $z_1 = a, z_2, \ldots, z_n,
    z_{n + 1} = b \in G$. Note that $P$ is compact, as it is the union of finitely many intervals.
    By theorem 5.17, we have $d(G \backslash \C, P) > 0$. For $k \in [n]$, define $f_k: [z_k, z_{k +
    1}] \to \R$ as $f_k(z) = |Re(z) - Re(z_k)| + |Im(z) - Im(z_k)|$, the Manhattan distance between
    $z$ and $z_k$ on the complex plane. 
  \end{proof}
\end{homeworkProblem}
